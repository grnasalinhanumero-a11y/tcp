...
def send_friend_request_single(uid, token, region="BR"):
...
async def create_detailed_start_packet(key, iv, region="BR"):
...
async def request_join_with_badge(target_uid, badge_value, key, iv, region="BR"):
...
async def bundle_packet_async(bundle_id, key, iv, region="br"):
...
async def safe_send_message(chat_type, message, target_uid, chat_id, key, iv, max_retries=3, region="br"):
...
async def get_player_name_from_uid(uid, region="BR"):
...
async def send_title_packet_direct(target_uid, chat_id, key, iv, region="br"):
...
async def set_bio_directly_async_with_retry(jwt_token, bio_text, region="BR", max_retries=3, retry_delay=2):
...
async def set_bio_directly_async(jwt_token, bio_text, region="BR"):
...
async def test_emote_packet(target_uid, emote_id, key, iv, region="BR"):
